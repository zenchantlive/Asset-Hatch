...
import { ChevronDown, ChevronRight, Play, Edit3, Loader2, RefreshCw } from 'lucide-react'
        ...
        >
            ...
            <div className="flex items-center justify-between mt-auto pt-2">
                ...
                <div className="flex items-center gap-1">
                    {/* Expand/Collapse Button (for Categories view) */}
                    {onExpandToggle && (
                        <Button
                            size="icon"
                            variant="ghost"
...
export function BatchPreviewContent({ selectedIds }: BatchPreviewContentProps) {
    const { parsedAssets, assetStates, generateImage } = useGenerationContext()
    const { toggleAssetSelection } = useGenerationLayout()
    ...
    const handleGenerateAll = async () => {
        for (const asset of selectedAssets) {
            if (assetStates.get(asset.id)?.status !== 'generating') {
                generateImage(asset.id)
            }
        }
    }
            ...
            <div className="flex flex-col h-full bg-black/20 text-white rounded-lg overflow-hidden animate-in fade-in duration-500">
                ...
                <div className="flex items-center justify-between p-4 border-b border-white/10 bg-white/5">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button
                            className="aurora-gradient text-white shadow-lg shadow-purple-900/20"
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            {assets.map(asset => (
                                                <AssetCard
                                                    ...
                                                    isExpandedItem={expandedAssetId === asset.id}
                                                    onClick={() => setExpandedAssetId(expandedAssetId === asset.id ? null : asset.id)}
                                                    onExpandToggle={() => setExpandedAssetId(expandedAssetId === asset.id ? null : asset.id)}
                                                />
                                            ))}
                                        </div>
                                    </div>
        ...
        <div className="flex flex-col h-full p-6 animate-in fade-in duration-500 overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex gap-2">
                    ...
                    <Button
                        onClick={handleGenerateAll}
                        className="aurora-gradient font-semibold text-white shadow-lg shadow-purple-900/20"
                    >
                        <Play className="w-4 h-4 mr-2" />
                        Generate All
                    </Button>
